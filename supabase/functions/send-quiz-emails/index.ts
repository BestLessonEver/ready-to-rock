...
const QuizSubmissionSchema = z.object({
  ...
  band: z.string().max(50),
  bandLabel: z.string().max(100),
  bandDescription: z.string().max(1000),
  primaryInstrument: z.string().max(100),
  secondaryInstruments: z.array(z.string().max(100)).max(5),
  actionPlan: z.array(z.string().max(500)).max(10),
  createdAt: z.string(),
  pitch: z.string().max(50).optional().default(""),
  rhythm: z.string().max(50).optional().default(""),
  memory: z.string().max(50).optional().default(""),
  emotionalResponse: z.string().max(50).optional().default(""),
  hummingSinging: z.string().max(50).optional().default(""),
  rhythmPlay: z.string().max(50).optional().default(""),
  dancing: z.string().max(50).optional().default(""),
  drawnToInstruments: z.string().max(50).optional().default(""),
  handlesCorrection: z.string().max(50).optional().default(""),
  performerStyle: z.string().max(50).optional().default(""),
  focusDuration: z.string().max(50).optional().default(""),
...
function generateLeadEmailHtml(submission: QuizSubmission, resultsUrl: string, formattedDate: string): string {
  const instrumentsAtHomeFormatted = (submission.instrumentsAtHome || [])
    .map(i => sanitizeForHtml(instrumentsAtHomeLabels[i] || i))
    .join(", ");
  ...
  return `
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <div style="background: #f8fafc; padding: 20px; border-radius: 8px; margin: 20px 0;">
        ...
        <p><strong>Parent:</strong> ${sanitizeForHtml(submission.parentName)}</p>
        <p><strong>Email:</strong> <a href="mailto:${sanitizeForHtml(submission.email)}">${sanitizeForHtml(submission.email)}</a></p>
        <p><strong>Phone:</strong> ${sanitizeForHtml(submission.phone || "Not provided")}</p>
        <p><strong>Child's Name:</strong> ${sanitizeForHtml(submission.childName)}</p>
      </div>
      ...
      <div style="background: #ecfdf5; padding: 20px; border-radius: 8px; margin: 20px 0;">
        ...
        <p style="font-size: 18px; color: #065f46; margin: 8px 0 0 0;"><strong>${sanitizeForHtml(submission.bandLabel)}</strong></p>
      </div>

      <div style="background: #eff6ff; padding: 20px; border-radius: 8px; margin: 20px 0;">
        <h2 style="color: #1e40af; margin-top: 0;">Instrument Recommendations</h2>
        <p><strong>Primary:</strong> ${sanitizeForHtml(submission.primaryInstrument)}</p>
        <p><strong>Also consider:</strong> ${submission.secondaryInstruments.map(i => sanitizeForHtml(i)).join(", ")}</p>
      ...
      <div style="background: #fefce8; padding: 20px; border-radius: 8px; margin: 20px 0;">
        ...
        <table style="width: 100%; border-collapse: collapse;">
          <tr><td style="padding: 4px 0;"><strong>Pitch:</strong></td><td>${sanitizeForHtml(getLabel("pitch", submission.pitch || ""))}</td></tr>
          <tr><td style="padding: 4px 0;"><strong>Rhythm:</strong></td><td>${sanitizeForHtml(getLabel("rhythm", submission.rhythm || ""))}</td></tr>
          <tr><td style="padding: 4px 0;"><strong>Memory:</strong></td><td>${sanitizeForHtml(getLabel("memory", submission.memory || ""))}</td></tr>
          <tr><td style="padding: 4px 0;"><strong>Emotional response:</strong></td><td>${sanitizeForHtml(getLabel("emotionalResponse", submission.emotionalResponse || ""))}</td></tr>
          <tr><td style="padding: 4px 0;"><strong>Humming/Singing:</strong></td><td>${sanitizeForHtml(getLabel("hummingSinging", submission.hummingSinging || ""))}</td></tr>
          <tr><td style="padding: 4px 0;"><strong>Rhythm play:</strong></td><td>${sanitizeForHtml(getLabel("rhythmPlay", submission.rhythmPlay || ""))}</td></tr>
          <tr><td style="padding: 4px 0;"><strong>Dancing:</strong></td><td>${sanitizeForHtml(getLabel("dancing", submission.dancing || ""))}</td></tr>
          <tr><td style="padding: 4px 0;"><strong>Drawn to instruments:</strong></td><td>${sanitizeForHtml(getLabel("drawnToInstruments", submission.drawnToInstruments || ""))}</td></tr>
          <tr><td style="padding: 4px 0;"><strong>Handles new things:</strong></td><td>${sanitizeForHtml(getLabel("handlesCorrection", submission.handlesCorrection || ""))}</td></tr>
          <tr><td style="padding: 4px 0;"><strong>Performer style:</strong></td><td>${sanitizeForHtml(getLabel("performerStyle", submission.performerStyle || ""))}</td></tr>
          <tr><td style="padding: 4px 0;"><strong>Focus duration:</strong></td><td>${sanitizeForHtml(getLabel("focusDuration", submission.focusDuration || ""))}</td></tr>
          <tr><td style="padding: 4px 0;"><strong>Wants to learn:</strong></td><td>${sanitizeForHtml(getLabel("wantsToLearn", submission.wantsToLearn || ""))}</td></tr>
          <tr><td style="padding: 4px 0;"><strong>Favorite song behavior:</strong></td><td>${sanitizeForHtml(getLabel("favoriteSongBehavior", submission.favoriteSongBehavior || ""))}</td></tr>
        </table>
      </div>
      ...
      <p style="color: #6b7280; font-size: 14px;">Submitted: ${sanitizeForHtml(formattedDate)}</p>
    </div>
  `;
}
  ...
  return `
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto;">
      <h1 style="color: #1a1a1a;">
        ${sanitizeForHtml(submission.childName)}'s Music Readiness Results Are In!
      </h1>
      ...
      <div style="background: #f8fafc; padding: 20px; border-radius: 8px; margin: 20px 0;">
        <h2 style="color: #1e40af; margin-top: 0;">${sanitizeForHtml(submission.bandLabel)}</h2>
        <p style="color: #374151; line-height: 1.6;">${sanitizeForHtml(submission.bandDescription)}</p>
      </div>
...
function isValidOrigin(req: Request): boolean {
  ...
  // In development, also check for missing headers (direct API calls from same origin)
  // For production, you may want to be stricter
  return false;
}
