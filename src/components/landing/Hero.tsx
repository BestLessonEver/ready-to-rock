...
export function Hero({ onStartQuiz }: HeroProps) {
  return (
    <section className="relative min-h-[85vh] flex items-center justify-center px-4 gradient-hero overflow-hidden">
      {/* Decorative elements */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-20 left-10 w-20 h-20 rounded-full bg-primary/10 animate-float" />
        <div className="absolute top-40 right-10 w-16 h-16 rounded-full bg-accent/20 animate-float" style={{ animationDelay: '1s' }} />
        <div className="absolute bottom-32 left-20 w-12 h-12 rounded-full bg-primary/15 animate-float" style={{ animationDelay: '2s' }} />
        <div className="absolute bottom-20 right-20 w-24 h-24 rounded-full bg-accent/10 animate-float" style={{ animationDelay: '0.5s' }} />
      </div>
      ...
      <div className="relative z-10 max-w-2xl mx-auto text-center space-y-8">
        ...
        {/* Subheadline */}
        <p className="text-lg sm:text-xl text-muted-foreground max-w-xl mx-auto animate-slide-up" style={{ animationDelay: '0.1s' }}>
          Take a quick quiz to get a personalized Music Readiness Score, 
          best-fit instrument, and a first-week success plan.
        </p>
        ...
        <p className="text-sm text-muted-foreground animate-fade-in" style={{ animationDelay: '0.4s' }}>
          Created by <span className="font-semibold text-foreground">Best Lesson Ever</span>, 
